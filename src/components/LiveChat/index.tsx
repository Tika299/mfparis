...
export const LiveChat = () => {
  ...
  const connectSocketForProfile = useCallback(async (profileId: string) => {
  ...
  const handleScroll = () => {
    if (!scrollRef.current || isLoadingMore || !hasMore || !profileRef.current?.id) return

    if (scrollRef.current.scrollTop < 10) {
      loadHistory(profileRef.current.id, page, true)
  ...
  const handleOpenChat = async () => {
    ...
    if (nextIsOpen) {
      if (profile?.id) {
        await connectSocketForProfile(profile.id)
      } else {
        await ensureSocket()
      }
    }
  ...
  const handleAuth = async (action: 'login' | 'register') => {
    ...
    if (data.success) {
      ...
      setStep('chat')
      loadHistory(data.user.id, 1)
      await connectSocketForProfile(data.user.id)
    } else {
      alert(data.error)
    }
  ...
  const handleLogout = () => {
    localStorage.removeItem('mf_chat_profile')
    setProfile(null)
    setStep('ask')
    setMessages([])
    setPage(1)
  ...
  return (
    <div className="relative font-sans antialiased">
      <button
        ...
        className={cn(
          'w-14 h-14 rounded-full shadow-2xl flex items-center justify-center transition-all relative',
          isOpen ? 'bg-white text-gray-500' : 'bg-[#b72828] text-white',
        )}
      >
        {isOpen ? <X size={24} /> : <MessageSquareDot size={30} />}
        {!isOpen && unreadCount > 0 && (
          <span className="absolute -top-1 -right-1 bg-red-600 text-white text-[10px] w-5 h-5 rounded-full flex items-center justify-center font-black border-2 border-white animate-bounce">
            {unreadCount}
          </span>
        )}
      </button>
