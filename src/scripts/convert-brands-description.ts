...
const INPUT_FILE = path.resolve(__dirname, 'brands.merged.json')
const OUTPUT_FILE = path.resolve(__dirname, 'brands.richtext.json')
...
async function main() {
    ...
    console.log('✅ Convert brands hoàn tất')
...
main().catch((error) => {
    console.error('❌ Convert brands thất bại:')
    console.error(error)
    process.exit(1)
})