...
const INPUT_FILE = path.resolve(__dirname, 'posts.json')
const OUTPUT_FILE = path.resolve(__dirname, 'posts.richtext.json')
...
async function main() {
    const posts = readJSON(INPUT_FILE)
    const editorConfig = await getEditorConfig()

    let convertedCount = 0

    const converted = posts.map((item) => {
        const contentHTML =
        ...
        const contentRichText = convertHTML(contentHTML, editorConfig)

        if (hasRichTextContent(contentRichText)) convertedCount++

        return {
            ...item,

            // Chỉ ghi đè field content
    ...
    console.log('✅ Convert posts content hoàn tất')
    console.log(`Tổng post: ${posts.length}`)
    console.log(`Có content richText: ${convertedCount}`)
    console.log(`Output: ${OUTPUT_FILE}`)
}

main().catch((error) => {
    console.error('❌ Convert posts content thất bại:')
    console.error(error)
    process.exit(1)
})